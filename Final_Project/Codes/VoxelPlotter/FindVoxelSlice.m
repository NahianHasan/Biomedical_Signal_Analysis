function [FV]=FindVoxelSlice(VoxelMat,Vox_Size)
...
FV.vertices=zeros(8*size(VoxelMat,2)*size(VoxelMat,3),3);
FV.faces=zeros(6*size(VoxelMat,2)*size(VoxelMat,3),4);
FaceIndex=1;
VertexIndex=0;
counter=1;
ExternalIndexes=zeros(size(VoxelMat,2)*size(VoxelMat,3),3);
...
for i=1:size(VoxelMat,1)
    for j=1:size(VoxelMat,2)
        for k=1:size(VoxelMat,3)
            if VoxelMat(i,j,k)~=0
                % if the current voxel is non zero
                if i==1 || j==1 || k==1 || i==size(VoxelMat,1) || j== size(VoxelMat,2) || k== size(VoxelMat,3)                 
                    % if a boundary voxel, needs to be plotted
                    ExternalIndexes(counter,1:3)=[i j k] ;
                    [FV,FaceIndex,VertexIndex]=FindExternalFaces(VoxelMat,ExternalIndexes,voxel_size,counter,FaceIndex,VertexIndex,Vox_Size,FV,VoxelMat(i,j,k));
                    counter=counter+1;                
                else
                    if VoxelMat(i+1,j,k)==0 || VoxelMat(i-1,j,k)==0 || VoxelMat(i,j+1,k)==0 || VoxelMat(i,j-1,k)==0 || VoxelMat(i,j,k+1)==0 || VoxelMat(i,j,k-1)==0
                        % if one of the neighbour is zero voxel, needs to
                        % be plotted
                        ExternalIndexes(counter,1:3)=[i j k] ;
                        [FV,FaceIndex,VertexIndex]=FindExternalFaces(VoxelMat,ExternalIndexes,voxel_size,counter,FaceIndex,VertexIndex,Vox_Size,FV,VoxelMat(i,j,k));
                        counter=counter+1;
...
FV.FaceVertexCData = FV.FaceVertexCData(any(FV.faces,2));
FV.FaceColor = 'flat';
% FV.EdgeColor = 'red';
...
function [FV,FaceIndex,VertexIndex]=FindExternalFaces(VoxelMat,ExternalIndexes,voxel_size,i,FaceIndex,VertexIndex,LOD,FV,value)
faces=[1 2 3 4; 2 6 7 3 ; 6 5 8 7; 5 1 4 8; 4 3 7 8 ; 1 2 6 5];
% 6 faces of the voxel in order
...
% storing the vertices of the current voxel
if ExternalIndexes(i,2)~=1
    if VoxelMat(ExternalIndexes(i,1),ExternalIndexes(i,2)-1,ExternalIndexes(i,3))==0
        ...
        FV.FaceVertexCData(FaceIndex,1)=value;        
